...
面向画师与数字艺术创作者的**本地化图像归属与验证工具**——可见威慑与可验证信号，提高盗用与洗图成本。

本仓库为精卫的**独立开源自托管版本**。所有图片计算均在您本地设备上完成，图片不离开本机、不上传任何云端服务器，处理完成后即刻从临时存储中销毁。

*(如需体验创作者社区、在线画廊与云端生态，可访问在线服务 [jwprotect.com](https://jwprotect.com)。自托管功能与官网可能不完全一致。)*

精卫采用分层防御体系：通过隐形频域与空间域归属信号（可在验证页解析），结合可选的可见威慑图层（如像素位移、微浮雕与动态网格），显著抬高 AI 去水印、局部重绘与随手盗图的成本。
...
| **闪卡预览（Holo-Card）** | [`/protect`](http://127.0.0.1:8080/protect) | 生成保护图后，支持展示用箔面/闪卡倾斜反光预览，并可一键录制导出 `.mp4` 短视频。 |
...
5. *(可选)* 在保护结果下方体验 **箔面/闪卡预览**，点击**下载闪卡视频 (.mp4)** 导出动态展示短片。
...
2. 上传后系统自动进行多层解码，展示 C2PA 声明（若包含且受支持）、JW 声明、追踪锚点、DWT 频域、LSB 隐写和 EXIF/IPTC 元数据。验证页仅**读取**已有且受支持的 C2PA 信息；本工具的保护流程**不签发、不写入、不生成** C2PA manifest。
3. 针对二次截屏或裁切后的图片，在追踪锚点处输入**保护时填写的同一段创作者署名**即可发起签名比对。

检出结果不构成法律所有权证明、真实性证明或 AI 来源证明。

## 支持精卫

自托管版免费使用。如果想资助后续开发，打赏会交给**精卫官方**，不是交给正在跑这份 Docker 的人：
